...
if chosen_radio == "Index":
    ...
    # save chosen_radio
    st.session_state['chosen_radio'] = "Valeur"
    ...
    # Astuce pour transformer le dictionnaire en json
    DFManu_json = pd.DataFrame.from_dict(DFManu, orient='index').T.to_json(orient="columns")

    if predict_btn_manu:
        st.session_state['valuesClient'] = DFManu

        http = urllib3.PoolManager()
        r = http.request('GET', path_server+'/predict/values'+'?values='+DFManu_json )
        if r.status != 200:
            st.write('Calcul du Score non disponible')
        elif json.loads(r.data)['proba'] != None:
            resData = json.loads(r.data)
